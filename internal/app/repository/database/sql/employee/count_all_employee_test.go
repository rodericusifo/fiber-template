...
func TestMysqlEmployeeDatabaseSQLRepository_CountAllEmployee(t *testing.T) {
	type (
		args struct {
			...
			before: func() {
				{
					...
					mockQuery.ExpectQuery(
						regexp.QuoteMeta(
							"SELECT `employees`.`id` FROM `employees` WHERE `employees`.`user_id` = ? AND `employees`.`deleted_at` IS NULL",
						),
					).
						WithArgs(arg1).
